/* ============================================
   Hip Recovery Tracker — App Logic
   ============================================ */

const STORAGE_KEY = 'hipRecovery.v2';

const DEFAULT_EXERCISES = [
  { id: uid(), name: 'Bike', type: 'time', sets: [{ reps: '', weight: '', duration: '', intensity: '' }] },
  { id: uid(), name: 'Hip machine', type: 'reps', sets: [{ reps: '', weight: '' }, { reps: '', weight: '' }, { reps: '', weight: '' }] },
  { id: uid(), name: 'Leg press (double)', type: 'reps', sets: [{ reps: '', weight: '' }, { reps: '', weight: '' }, { reps: '', weight: '' }] },
  { id: uid(), name: 'Leg press (single)', type: 'reps', sets: [{ reps: '', weight: '' }, { reps: '', weight: '' }, { reps: '', weight: '' }] },
];
...
function renderHome() {
  ...
  // Bottom nav
  tpl.querySelectorAll('.nav-item').forEach(btn => {
...
function renderNewSession() {
  ...
  const session = {
    ...
    exercises: DEFAULT_EXERCISES.map(ex => ({
      ...
      name: ex.name,
      type: ex.type,
      sets: ex.sets.map(() => ({ reps: '', weight: '', duration: '', intensity: '' }))
    })),
  ...
  tpl.querySelector('[data-add-exercise]').onclick = () => {
    const ex = {
      ...
      name: '',
      type: 'reps',
      sets: [{ reps: '', weight: '', duration: '', intensity: '' }]
    };
  ...
  tpl.querySelector('[data-save-session]').onclick = () => {
    ...
    // Filter out empty sets within each exercise
    session.exercises.forEach(ex => {
      ex.sets = ex.sets.filter(s => {
        if (ex.type === 'time') {
          return s.duration || s.intensity;
        }
...
function buildExerciseRow(exercise, session) {
  ...
  const repsContainer = row.children[3]; // First sets-container
  const timeContainer = row.children[4]; // Second sets-container
  const setsListReps = repsContainer.querySelector('[data-sets-list]');
  const setsListTime = timeContainer.querySelector('[data-sets-list-time]');
  ...
  function setType(type) {
    ...
    repsContainer.classList.toggle('hidden', type !== 'reps');
    timeContainer.classList.toggle('hidden', type !== 'time');
  }
  ...
  // Add set buttons
  const addSetBtn = repsContainer.querySelector('[data-add-set]');
  const addSetTimeBtn = timeContainer.querySelector('[data-add-set-time]');

  addSetBtn.onclick = () => {
    const newSet = { reps: '', weight: '' };
    exercise.sets.push(newSet);
    setsListReps.appendChild(buildSetRow(newSet, exercise, 'reps'));
    updateSetNumbers(setsListReps);
  };

  addSetTimeBtn.onclick = () => {
    const newSet = { duration: '', intensity: '' };
    exercise.sets.push(newSet);
    setsListTime.appendChild(buildSetRow(newSet, exercise, 'time'));
    updateSetNumbers(setsListTime);
  };

  // Render initial sets
  if (exercise.type === 'time') {
    exercise.sets.forEach(set => {
      setsListTime.appendChild(buildSetRow(set, exercise, 'time'));
    });
  } else {
    exercise.sets.forEach(set => {
      setsListReps.appendChild(buildSetRow(set, exercise, 'reps'));
    });
  }
...
function buildSetRow(setData, exercise, format) {
  const templateId = format === 'time' ? 'tpl-set-row-time' : 'tpl-set-row';
  const tpl = document.getElementById(templateId).content.cloneNode(true);
  const row = tpl.querySelector('[data-set-row]');

  // Set number
  const setIndex = exercise.sets.indexOf(setData);
  row.querySelector('[data-set-number]').textContent = setIndex + 1;

  if (format === 'reps') {
    const repsInput = row.querySelector('[data-set-reps]');
    repsInput.value = setData.reps || '';
    repsInput.oninput = (e) => setData.reps = e.target.value;

    const weightInput = row.querySelector('[data-set-weight]');
    weightInput.value = setData.weight || '';
    weightInput.oninput = (e) => setData.weight = e.target.value;
  } else {
    const durationInput = row.querySelector('[data-set-duration]');
    durationInput.value = setData.duration || '';
    durationInput.oninput = (e) => setData.duration = e.target.value;

    const intensityInput = row.querySelector('[data-set-intensity]');
    intensityInput.value = setData.intensity || '';
    intensityInput.oninput = (e) => setData.intensity = e.target.value;
  }

  row.querySelector('[data-remove-set]').onclick = () => {
    exercise.sets = exercise.sets.filter(s => s !== setData);
    row.remove();
    updateSetNumbers(row.parentElement);
  };
...
function updateSetNumbers(container) {
  const rows = container.querySelectorAll('[data-set-number]');
    ...
    session.exercises.forEach(ex => {
      if (ex.type === 'time') {
        ex.sets.forEach((s, i) => {
          ...
          row.innerHTML = `<strong>${i === 0 ? escapeHtml(ex.name) : ''}</strong><span>Set ${i + 1}: ${detail}</span>`;
...
function renderHistory() {
  ...
  tpl.querySelector('[data-back]')?.remove();
...
function buildSessionCard(session) {
  ...
  metaEl.textContent = `${setCount} set${setCount === 1 ? '' : 's'} · ${statusText}`;
...
function buildChart(sessions) {
  ...
  [0, 5, 10].forEach(v => {
    const y = yFor(v) + P;
    grid += `<line x1="${P}" y1="${y}" x2="${W - P}" y2="${y}" stroke="#e2dfda" stroke-width="1" />`;
    grid += `<text x="${P - 6}" y="${y + 3}" text-anchor="end" font-size="9" fill="#999">${v}</text>`;
  });
...
function renderPrint() {
  ...
  tpl.querySelector('[data-export-pdf]').onclick = exportPDF;
...
function buildPrintSession(session) {
  ...
  if (session.exercises.length > 0) {
    ...
    // Flatten exercises with multiple sets into separate rows
    const rows = [];
    session.exercises.forEach(ex => {
      ex.sets.forEach((set, setIdx) => {
        if (ex.type === 'time') {
          rows.push({
            name: setIdx === 0 ? ex.name : '',
            setNum: setIdx + 1,
            reps: '—',
            weight: set.duration || '—',
        ...
        <tbody>
          ${rows.map(r => `
            <tr>
              ...
              <td class="num">${r.isTime ? '' : r.setNum}</td>
...
try {
  ...
  document.getElementById('app').innerHTML = `<div class="empty-state">Error: ${e.message}</div>`;
}